...
// TODO: BIG NUMBER OF THANKS FOR THIS MESSAGE
...
class ThankfulPeoplePlugin extends Gdn_Plugin {
	...
	public $CommentGroup = array();
	public $DiscussionData = array();
	public $Session;
...
/*   public function DiscussionController_AfterCommentMeta_Handler(&$Sender) {
	...
	protected function _AttachPostCount(&$Sender) {
		...
		echo '<div class="ThankCount">'.sprintf(T(Plural($Posts,'Thanks: %s','Thanks: %s')),number_format($Posts,0)).'</div>';
	}
	*/
	...
	public function DiscussionController_Render_Before($Sender) {
		...
		$Sender->AddJsFile('plugins/ThankfulPeople/js/dev.jquery.expander.js');
	...
	public function DiscussionController_AfterCommentBody_Handler($Sender) {
		...
		switch ($Type) {
			case 'Comment': {
				if ($Object->ThankCount <= 0) return; // up
				$ThankedByCollection = GetValue($Object->CommentID, $this->CommentGroup);
				if ($ThankedByCollection) { // TODO: USE GetThankedByList() here
					$ThankedByList = implode(' ', array_map('UserAnchor', $ThankedByCollection));
				}
			} break;
			case 'Discussion': {
				if ($Object->ThankCount <= 0) return; // up
				$ThankedByList = implode(' ', array_map('UserAnchor', $this->DiscussionData)); // TODO: USE GetThankedByList() here
			} break;
			default: throw new Exception('What...');
		}
		//$ThankedByList = trim(trim($ThankedByList), ',');
		if ($ThankedByList != '') {
	...
	public function UserInfoModule_OnBasicInfo_Handler($Sender) {
		...
		echo Wrap($User->ReceivedThankCount, 'dd', array('class' => 'ReceivedThankCount'));
	}
